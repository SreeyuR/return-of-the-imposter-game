...
/**
 * Removes all bodies and forces from the scene.
 *
 * @param scene a pointer to a scene returned from scene_init()
 */
...
/**
 * Detects if a body is "visible" from the point of view of another body.
 *
 ...
 * @param opaqueness_predicate a function pointer that determines whether a
 * given body (not body1 or body2, but an intermediate body) in a scene is
 * opaque to the line of sight between body1 and body2. If this param is NULL,
 * then all bodies in the scene are considered opaque.
 *
 * @return true if body2 is visible from body1's point of view, else false
 */
bool scene_detect_line_of_sight(scene_t *scene, body_t *body1, body_t *body2,
                                body_predicate_t opaqueness_predicate);
